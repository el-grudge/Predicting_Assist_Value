...
# LIGA <- dplyr::filter(Matches, competition.competition_name == unique(Matches$competition.competition_name)[4])
# FAWSL <- dplyr::filter(Matches, competition.competition_name == unique(Matches$competition.competition_name)[2])
# CHAMPIONS <- dplyr::filter(Matches, competition.competition_name == unique(Matches$competition.competition_name)[1])
# WORLDCUP <- dplyr::filter(Matches, competition.competition_name == unique(Matches$competition.competition_name)[3])
# NWSL <- dplyr::filter(Matches, competition.competition_name == unique(Matches$competition.competition_name)[5])
# PREMIER <- dplyr::filter(Matches, competition.competition_name == unique(Matches$competition.competition_name)[6])
# WWORLDCUP <- dplyr::filter(Matches, competition.competition_name == unique(Matches$competition.competition_name)[7])

library(dplyr)
# length(LIGA$match_id)/4
# LIGA$match_id[1:113]
# LIGA$match_id[114:226]
# LIGA$match_id[227:339]
# LIGA$match_id[340:452]
ALLEvents <- data.frame()

# for (i in LIGA$match_id[1:113]){
# for (i in LIGA$match_id[114:226]){
# for (i in LIGA$match_id[227:339]){
for (i in LIGA$match_id[340:452]){
  print(i)
  event <- select(get.matchFree(filter(LIGA, match_id == i)), all_of(colkeys))
  event$match_name <- paste(filter(LIGA, match_id == i)$home_team.home_team_name,
                            'v',
                            filter(LIGA, match_id == i)$away_team.away_team_name) 
  event$match_date <- filter(LIGA, match_id == i)$match_date
  ALLEvents <- rbind(ALLEvents, event)
}

dim(ALLEvents) # [1] 1301868      84
